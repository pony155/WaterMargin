...
transforms. The current WPF sprite example references the runtime assembly but
does not yet select a locale or display localized UI.
...
CMake exposes `GameLocalizationRuntime`, `GameLocalizationCompiler`,
`GameLocalizationCatalogs`, and (when `BUILD_TESTING` is enabled)
`GameLocalizationUnitTests`. The last target compiles the headless localization
test executable only. CTest registers it as `GameLocalizationTests`, but CI/CD
owns execution under repository policy.
