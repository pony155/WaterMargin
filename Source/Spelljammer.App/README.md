...
| `SpriteForgeMainMenuView.cs` | Current main-menu surface. It loads the packaged `Background.png`, draws it edge-to-edge with aspect-preserving cover scaling, and uses a fixed 1280x720 logical UI canvas. Menu grouping and button fills are transparent, leaving localized New Game, Game Settings, and Quit Game labels, the bottom-right application version, and one synchronized hover/keyboard selection outline. Pointer movement advances SpriteForge focus to the hovered copied element bound, so keyboard navigation and acceptance continue from the same item. SpriteForge owns retained elements, action hit testing, focus, input processing, and action generation; WPF realizes copied presentation commands, localized text, and the outline. The view emits only stable top-level actions, keeps bounded element/action buffers, and releases its native UI context deterministically. |
