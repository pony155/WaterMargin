...
| `SpriteForgeCharacterCreationView.cs` | Character-creation surface on a 1100x680 logical canvas. SpriteForge owns its transactionally created modal tree, five-button focus order, stable-key focus restoration, hit testing, pointer/keyboard actions, and tagged/clipped presentation. WPF draws the captain monogram and localized captain, lineage, heritage, Background, summary, and explicit seed. Previous/Next cycles the bounded authored templates, Reroll replaces the draft seed, Escape/Back cancels, and Confirm emits a copied selection. |
