...
Compile a catalog from the WaterMargin repository root:
...
CMake exposes `WaterMarginLocalizationRuntime`,
`WaterMarginLocalizationCompiler`, `WaterMarginLocalizationCatalogs`, and
(when `BUILD_TESTING` is enabled) `WaterMarginLocalizationUnitTests`. The last
target compiles the headless localization test executable only. CTest registers
it as `WaterMarginLocalizationTests`, but CI/CD owns execution under repository
policy.
