...
| Owner | `Game` with Game-owned offline tooling |
...
`Game/` still contains a Windows-only .NET 10 WPF sprite-rendering example, not
the RTS application. Phases 1 and 2 provide a separate Game-owned .NET 10
...
| Area | Authority |
| --- | --- |
| Game/application ownership and large-battle constraints | [Game design](../GameDesign.md) |
| UI layout, actions, semantics, and managed boundary | [UI system](../../Engine/Docs/UISystem.md) |
| Unicode shaping, font fallback, glyph coverage, and bidi limits | [Font system](../../Engine/Docs/FontSystem.md) |
| Stable asset identity and transactional publication | [Asset system](../../Engine/Docs/AssetSystem.md) |
| Deterministic simulation and replay commands | [Command system](../../Engine/Docs/CommandSystem.md) |
| Application lifecycle and service ordering | [Framework design](../../Engine/Docs/Frameworkd.md) |
| Dialogue, UI, and accessibility audio routing | [Audio system](../../Engine/Docs/AudioSystem.md) |
...
The first compiler belongs under `Game/Tools/Localization` because there is not
yet a second credible product requiring a generic engine localization service.
...
example, a battle result owns faction/unit/regiment IDs and numeric casualties;
presentation resolves their display names and formats one complete message.
No simulation component stores the resulting sentence.
...
These paths are proposed. No empty runtime/tool directories or targets should
be created before their phase is implemented with tests and documentation.

## Architecture

```mermaid
flowchart LR
    Source[Source catalog + translator metadata] --> Compiler[Game localization compiler]
...
[`cldr-48.2.0-phase2.json`](../Content/Localization/cldr-48.2.0-phase2.json)
and [Unicode data notice](../Content/Localization/UNICODE-LICENSE-NOTICE.md).
