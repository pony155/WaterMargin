...
public sealed class EngineViewport : HwndHost
{
    ...
    protected override HandleRef BuildWindowCore(HandleRef hwndParent)
    {
        ...
        try
        {
            uint interopVersion = EngineNative.SpriteForge_GetInteropVersion();
            if (interopVersion != ExpectedInteropVersion)
            {
                throw new InvalidOperationException(
                    $"SpriteForge.dll exposes interop version {interopVersion}; " +
                    $"the Game expects {ExpectedInteropVersion}.");
            }

            ThrowIfFailed(
                EngineNative.SpriteForge_CreateRenderer(
                    childWindow, 320, 180, 16, 1, out renderer),
                "create the renderer");

            byte[] pixels = BuildSpriteSheet();
            ThrowIfFailed(
                EngineNative.SpriteForge_CreateRgba8Texture(
                    renderer,
                    SheetWidth,
                    FrameHeight,
                    pixels,
    ...
    private void RenderScene()
    {
        ...
        EngineStatus status = EngineNative.SpriteForge_RenderSprites(
    ...
    private void ReleaseEngineResources()
    {
        ...
        if (spriteSheet != 0)
        {
            EngineNative.SpriteForge_DestroyTexture(renderer, spriteSheet);
            spriteSheet = 0;
        }
        EngineNative.SpriteForge_DestroyRenderer(renderer);
        renderer = nint.Zero;
    }
